...
int main(int argc, char** argv)
{
	...
	for(int i = 0; dynamic[i].d_tag != DT_NULL; i++)
	{
		...
		{
			dynsym = (Elf64_Sym *)((unsigned long)dynamic[i].d_un.d_val + (unsigned long)so);
	...
	for(int i = 0; ;i++) 
	{
		...
		{
			ReflectiveLoader = (void *)dynsym[i].st_value;
	...
	printf("[+] Returned from Stage 0 shell code RIP of target is %llx\n", mmap_regs.rip);
	printf("[i] Stage 0 mmap returned memory address of %llx.. verifying allocation succeeded..\n", mmap_regs.rax);

	if(isRWX(target, (void *)mmap_regs.rax) == -1)
	{
	...
	void *so_inject_addr = (void *)mmap_regs.rax;
	...
	regs.rip = (unsigned long)((unsigned long)ReflectiveLoader + (unsigned long)so_inject_addr);
	ptrace_setregs(target, &regs);
